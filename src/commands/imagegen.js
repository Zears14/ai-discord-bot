...
class ImageGenCommand extends BaseCommand {
  constructor(client) {
    super(client, {
      name: 'imagegen',
      description:
        'Generate images using AI. Use "|" to separate the prompt from the negative prompt.',
      category: 'AI',
  ...
  async execute(message, args) {
    ...
    if (!fullPrompt) {
      return message
        .reply(CONFIG.EMBED.EMPTY_IMAGE_PROMPT)
        .catch((err) =>
          logger.discord.error(`Failed to send empty prompt message: ${err.message}`)
        );
    }
    ...
    try {
      ...
      const attachment = new AttachmentBuilder(imageBuffer, {
        name: CONFIG.IMAGE_GEN.IMAGE_OUTPUT.FILENAME,
      });
      ...
      logger.discord.cmdError('Image generation command error:', error);
      if (loadingMessage) {
