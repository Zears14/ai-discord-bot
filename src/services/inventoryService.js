...
async function removeItemFromInventory(userId, guildId, itemId, quantity = 1) {
    const currentItem = await pool.query('SELECT quantity FROM inventory WHERE userid = $1 AND guildid = $2 AND itemid = $3', [userId, guildId, itemId]);
    ...
    if (currentItem.rows[0].quantity === quantity) {
        const query = 'DELETE FROM inventory WHERE userid = $1 AND guildid = $2 AND itemid = $3 RETURNING *;';
...
async function hasItem(userId, guildId, itemId, quantity = 1) {
    const query = 'SELECT quantity FROM inventory WHERE userid = $1 AND guildid = $2 AND itemid = $3;';
    const res = await pool.query(query, [userId, guildId, itemId]);
    if (res.rowCount === 0) {
        return false;
    }
